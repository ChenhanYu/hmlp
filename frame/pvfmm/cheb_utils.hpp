...
template <class ValueType, class Derived> class BasisInterface {
 ...
 public:
  template <Integer DIM> static void Nodes(Integer order, Vector<ValueType>& nodes) {
    ...
    Integer order_DIM = pvfmm::pow<Integer>(order, DIM);
  ...
  template <Integer DIM> static void Approx(Integer order, const Vector<ValueType>& fn_v, Vector<ValueType>& coeff) {
    ...
    {  // Precompute
      ...
      PVFMM_ASSERT(order < precomp.Dim());
    ...
    Integer order_DIM = pvfmm::pow<Integer>(order, DIM);
    Integer order_DIM_ = pvfmm::pow<Integer>(order, DIM - 1);
    Long dof = fn_v.Dim() / order_DIM;
    PVFMM_ASSERT(fn_v.Dim() == dof * order_DIM);
  ...
  template <Integer DIM> static void Eval(Integer order, const Vector<ValueType>& coeff, ConstIterator<Vector<ValueType>> in_x, Vector<ValueType>& out) {
    ...
    PVFMM_ASSERT(coeff.Dim() == Ncoeff * dof);
    ...
    {  // Rearrange coefficients into a tensor.
      Vector<ValueType> tensor(dof * pvfmm::pow<Integer>(order, DIM), buff1, false);
    ...
    for (Integer k = 0; k < DIM; k++) {  // Apply Mp along k-dimension
      Integer order_DIM = pvfmm::pow<Integer>(order, DIM - k - 1);
  ...
  template <Integer DIM> static ValueType TruncErr(Integer order, const Vector<ValueType>& coeff) {
    ...
    for (Long l = 0; l < dof; l++) {  // TODO: optimize this
      ...
      while (1) {
        if (sum < order) {
          if (sum == order - 1) err += pvfmm::fabs<ValueType>(coeff[offset0 + indx0]);
          indx0++;
        }
        indx1++;
        sum++;
  ...
  template <Integer DIM> static void Grad(Integer order, const Vector<ValueType>& coeff_in, Vector<ValueType>* coeff_out) {
    ...
    PVFMM_ASSERT(coeff_in.Dim() == Ncoeff * dof);
    ...
    // Create work buffers
    Long buff_size = dof * pvfmm::pow<Integer>(order, DIM);
    ...
    for (Integer k = 0; k < DIM; k++) {  // buff2 <-- Grad(buff0)
      Long N0 = pvfmm::pow<Integer>(order, k);
      Long N1 = order;
      Long N2 = pvfmm::pow<Integer>(order, DIM - k - 1);
  ...
  template <Integer DIM, Integer SUBDIM> static void Integ(Matrix<ValueType>& Mcoeff, Integer order, ConstIterator<ValueType> trg_, ValueType side, Integer src_face, const KernelFunction<ValueType, DIM>& ker, ValueType tol = -1, Integer Nq = 0) {
    ...
    while (err > tol) {
      err = 0;
      ValueType max_val = pvfmm::pow<SUBDIM>(side);
      ...
      for (Integer i = 0; i < Mtmp.Dim(0) * Mtmp.Dim(1); i++) {
        err = std::max(err, pvfmm::fabs<ValueType>(Mtmp[0][i] - Mcoeff[0][i]));
        max_val = std::max(max_val, pvfmm::fabs<ValueType>(Mtmp[0][i]));
      }
      err /= max_val;
      Mcoeff = Mtmp;
      if (Nq>200) {
        PVFMM_WARN("Failed to converge, error = "<<err);
        break;
      }
    }
  ...
  template <Integer DIM> static void tensor2coeff(Integer order, const Vector<ValueType>& tensor, Vector<ValueType>& coeff) {
    Integer Ncoeff = 1, Ntensor = pvfmm::pow<Integer>(order, DIM);
    for (Integer i = 0; i < DIM; i++) Ncoeff = (Ncoeff * (order + i)) / (i + 1);
    Long dof = tensor.Dim() / Ntensor;
    PVFMM_ASSERT(tensor.Dim() == Ntensor * dof);
  ...
  template <Integer DIM> static void Truncate(Vector<ValueType> &coeff0, Integer order0, Integer order1) {
    ...
    PVFMM_ASSERT(coeff0.Dim() == Ncoeff0 * dof);
  ...
  template <Integer DIM, Integer CONTINUITY> static void MakeContinuous(Vector<ValueType> &coeff0, Vector<ValueType> &coeff1, Integer order, Integer dir0, Integer dir1) {
    ...
    PVFMM_ASSERT(coeff0.Dim() == Ncoeff * dof);
    PVFMM_ASSERT(coeff1.Dim() == Ncoeff * dof);

    static Matrix<Matrix<ValueType>> M(2*DIM, 2*DIM);
    if (M[dir0][dir1].Dim(0) != 2 * Ncoeff) {
      Integer Ngrid = pvfmm::pow<Integer>(order, DIM - 1);
      ...
      auto fn_perturb = [&](std::function<ValueType(ValueType)> fn, bool even) { // Set M0
        ...
        M0.SetZero();
        { // dir0
          Integer N0=pvfmm::pow<Integer>(order, dir0/2);
          Integer N1=pvfmm::pow<Integer>(order, 1);
          Integer N2=pvfmm::pow<Integer>(order, DIM - dir0/2 - 1);
          PVFMM_ASSERT(N0 * N2 == Ngrid);
        ...
        { // dir1
          Integer N0=pvfmm::pow<Integer>(order, dir1/2);
          Integer N1=pvfmm::pow<Integer>(order, 1);
          Integer N2=pvfmm::pow<Integer>(order, DIM - dir1/2 - 1);
          PVFMM_ASSERT(N0 * N2 == Ngrid);
          Vector<ValueType> val(Ngrid * order), coeff;
          val.SetZero();
          for (Integer i0=0;i0<N0;i0++){
      ...
      //{ // Set Mgrid2coeff
      //  Integer Ngrid = pvfmm::pow<Integer>(order, DIM);
      ...
      //      if(i+j<order) coeff_weight.PushBack(pvfmm::pow<ValueType>(1.5, i+j)*1e-4);
      //    }
      //  }
      //  PVFMM_ASSERT(coeff_weight.Dim()==Ncoeff);
 ...
 //private:
  ...
  static void cheb_nodes_1d(Integer order, Vector<ValueType>& nodes) {
    ...
    for (Integer i = 0; i < order; i++) {
      nodes[i] = -pvfmm::cos<ValueType>((i + (ValueType)0.5) * pvfmm::const_pi<ValueType>() / order) * 0.5 + 0.5;
    }
  }
  ...
  template <Integer DIM, Integer SUBDIM> static void Integ_(Matrix<ValueType>& Mcoeff, Integer order, ConstIterator<ValueType> trg_, ValueType side, Integer src_face, const KernelFunction<ValueType, DIM>& ker, Integer Nq = 0) {
    ...
    {                                 // Set perm0
      PVFMM_ASSERT(0 <= src_face && src_face < 2 * DIM);
      if (SUBDIM == DIM - 1) {
        for (Integer i = 0; i < DIM; i++) {
    ...
    Matrix<ValueType> Mtensor(kdim[1] * kdim[0], pvfmm::pow<Integer>(order, SUBDIM));
    Mtensor.SetZero();
